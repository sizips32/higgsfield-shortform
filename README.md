...
|---|---|---|---|
| 1 | 기획 (컨셉·훅) | 승인 | 무료 |
| 2 | 시나리오 (샷리스트·캐릭터 일관성 보드·보이스 시트) | 승인 | 무료 |
| 3 | 스타일·키프레임 | 톤·캐릭터 유지 확인 | 소액(이미지) |
| 4 | 영상 프롬프트 (characterContinuity·voiceTag 유지) | 자동 | 무료 |
| 5 | 렌더 | **비용 게이트** | 크레딧(영상) |
| 6 | 큐레이션 | **인간 안목** | virality 소액(옵션) |
| 7 | 후반 체크리스트 | 없음 | 무료 |

캐릭터가 반복 등장하면 2단계 산출물에 **캐릭터 일관성 보드**와 **보이스 시트**를 포함한다. 각 인물은 `voiceTag`로 구별하고, 이후 프롬프트에는 `characterContinuity`와 캐릭터별 `voiceProfile`을 반복 주입한다.

옵션: `short-consistency` — MCP 참조 미디어와 보이스 시트로 반복 인물의 얼굴·의상·음성 구별을 보강.
